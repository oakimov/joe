...
/* Path A: mark globals + mark stack + mark set/goto/toggle live in
 * `src/ublock.zig` (`nowmarking`/`square`/`lightoff`/`markb`/`markk`/
 * `nstack`/`autoswap`, `upsh`/`upop`/`markv`, `umarkb`…`utomarkbk`).
 */
...
/* Path A: mark commands live in src/ublock.zig. */
