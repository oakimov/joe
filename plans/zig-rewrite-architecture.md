...
- 🚧 `src/render/{root,lgen,gap,attr,syntax,view}.zig` — `lgen*` + `GapBuffer`/`Point` + per-byte `attr_buf` apply + Zig-native JSF subset DFA (`load`/`loadWithLibrary`/`parseLine`/`stateAfterLines`; `conf.jsf` + keywords/`buffer`/`strings`/`"&"` + local/external `call=`/`return` + `.ifdef` params + `reset` + mark/recolormark + `\i`/`\c` + delimiter `save_*`/`push_*`/`pop_*`/`%`/`&`) + Zig-native `lgen_view` Feature 1.x (`ViewTables` hide/substitute/link_url/col_map; headings/fences/HR/blockquote/task/emphasis/links; `Options.view` + `paintBody`/`TextWindow.viewmode` via stack `bindScratch`); `TextWindow.buffer`/`syntax`/`line_attrs`/`viewmode` + `paintBody` auto-fill; `zig build render-test` **51/51**; `window-test` **89/89**; still no table box-drawing/OSC 8/`hold`/lattr cache/hybrid-B/live joe wiring
...
- ⬜ `lgen_view` deepen: table box-drawing, OSC 8 hyperlink emit, `hold`, lattr-style state cache
