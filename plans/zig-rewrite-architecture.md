...
- 🚧 `src/render/{root,lgen,gap,attr,syntax}.zig` — `lgen*` + `GapBuffer`/`Point` + per-byte `attr_buf` apply + Zig-native JSF subset DFA (`load`/`parseLine`/`stateAfterLines`; enough for `conf.jsf`: colors/states/`*`/`""`/`noeat`/`recolor`); `TextWindow.buffer`/`syntax`/`line_attrs` + `paintBody` auto-fill; `zig build render-test` **23/23**; `window-test` **88/88**; no keywords/call/mark/`lgen_view`/hybrid-B yet
...
- ⬜ JSF deepen: keywords/buffer/call/rtn/mark/delimiter/`%`/`&`/classes + lattr-style state cache
