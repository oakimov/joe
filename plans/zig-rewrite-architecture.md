...
- ✅ gated hybrid `scrn` swap — `JOE_ZIG_SCREEN_SWAP` / `zig_screen_swap_enabled` (default off); paint (`outatr`/`eraeol`/scroll) updates hybrid shadow only; `edupd` calls `zig_scrn_swap_flush` → `syncHybridGridToScreen` → `Screen.flush` → `drainZigScreen`; companion Screen lifecycle in `nresize`/`nclose`
...
- ✅ Screen buffer diff + dirty-cell-only flush — redesign `Screen.flush` cell-diff + EL + within-line ICH/DCH magic + gated hybrid `scrn` swap landed (default off)
