...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect + follow + post-edit scroll + `lgen_view` inline (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.4–1.6); still C: `lgen_view` line-start features (heading/fence/blockquote/HR/task) + Feature 1.9 + lifecycle helpers in `joe/bw.c`
