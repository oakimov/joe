...
- ✅ Native text-body via Phase 6 start: `TextWindow.body_lines`/`top_line`/`offset` (display col)/`tab`/`cursor_line`/`cursor_col`/`linums` + `contentCursor`/`bodyLine` + `paint.paintBody` → `render.lgenLine` + `paintLinum` (tabs/UTF-8/controls/wide clip; unit-tested; no gapbuffer/syntax/viewmode yet).
...
- 🚧 `src/render/{root,lgen}.zig` — minimal `lgenLine` over UTF-8 slices (tab expand, display-column `offset`, C0/DEL caret glyphs, wide-char `>` clip); `paintBody` wired; `zig build render-test` **8/8**; no gapbuffer/syntax/`lgen_view` yet
- ⬜ `lgen_core` over live gapbuffer `P` + syntax `attr_buf`
