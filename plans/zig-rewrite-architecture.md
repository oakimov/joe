...
### Phase 7: File I/O & Commands (3-4 weeks) — IN PROGRESS (Path A uedit + ublock + uformat + ushell + utag + umath + uisrch + usearch + uerror + ufile + tab + path + poshist done)
...
- ✅ Path A `poshist` zero-C: Zig `src/poshist.zig` owns position history (`afterpos`/`aftermove`/`windie`/`uprevpos`/`unextpos`); `joe/poshist.c` removed from the link (tombstone); soak **196/196**; next Phase 7: remaining C (`help`/`mouse`/`gettext`/`builtins`/`vt`/`state`/`cclass`/`selinux`/`unicat`/`main`) or gapbuffer `fileio` hardening
