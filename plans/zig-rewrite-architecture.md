...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect + follow + post-edit scroll (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel`); still C: remaining `lgen_view` chrome + lifecycle helpers (`bwmk`/`bwmove`/`bwresz`/…) in `joe/bw.c`
