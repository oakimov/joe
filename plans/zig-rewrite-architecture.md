...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect + follow (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*`); still C: remaining `lgen_view` chrome + `bwins`/`bwdel` / other window helpers in `joe/bw.c`
