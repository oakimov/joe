...
- ✅ `terminal/terminfo.zig` — Zig-native terminfo binding (`setupterm`/`tiget*`/`tiparm` + cached rare caps incl. `el`/`ed` + `formatCup`/`formatCsr`/`formatIl`/`formatDl`; unit-tested; not wired into live joe yet)
...
- ✅ `terminal/screen.zig` — Zig-native cell grid + dirty-row flush via ANSI/terminfo-cup + UTF-8 `writeText` with display-width + `Attribute`/`Color` (indexed + truecolor SGR) + alt-screen/keypad + scroll-region/IL/DL + clear-to-EOL/EOS helpers (obuf-style out buffer; cell-diff deferred to Phase 6; unit-tested)
