...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect + follow + post-edit scroll + full `lgen_view` Feature 1.x + thin `lgen_view` dispatcher/entry + lifecycle (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.3–1.10 / `bwmk`/`bwmove`/`bwresz`/`bwrm`/`orphit`/`calclincols`); still C: viewmode static storage + Feature fallback chrome + remaining non-paint helpers in `joe/bw.c` (`file_pos*` / `ustat` / `ucrawl*` / `vtmaster` / `init_visiblews` / …)
