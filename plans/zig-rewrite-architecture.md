...
- ✅ Native shallow text-body stub: `TextWindow.body_lines`/`top_line`/`offset`/`cursor_line`/`cursor_col`/`linums` + `contentCursor`/`bodyLine` + `paint.paintBody`/`paintLinum` (bwgen/gennum/disptw curx scaffold; unit-tested; real `lgen`/gapbuffer still Phase 6/`render`).
