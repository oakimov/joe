...
- 🚧 Phase 5 redesign: Zig-native `src/window/{root,screen,tw,pw,qw,menu}.zig` — Screen layout + grow/shrink/split/showAll/help-wind + `tw.stagen` status-line (`%n`/`%r`/`%c`/…); `zig build window-test` **22/22**; not wired into live joe
...
- 🚧 redesign `src/window/{root,screen,tw,pw,qw,menu}.zig` — Zig-native window types (Screen layout + grow/shrink/splitText/showAll/setHelpLines + `tw.stagen`/`composeStatus` status-line; `zig build window-test` **22/22**; hybrid ports remain live)
