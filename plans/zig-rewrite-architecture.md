...
### Phase 7: File I/O & Commands (3-4 weeks) — IN PROGRESS (Path A uedit + ublock + uformat + ushell + utag + umath done)
...
- ✅ Path A `umath` zero-C: Zig `src/umath.zig` owns `calc`/`umath`/`usmath`/`joe_strtod`/`math_cmplt` + `merr`/`mathhist` (translate-c Path A); `joe/umath.c` removed from the link (tombstone); soak **196/196**; next Phase 7: search/isrch/error nav
