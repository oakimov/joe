...
- ✅ Native text-body via Phase 6: `TextWindow.body_lines` or live `buffer: *GapBuffer`/`line_attrs`/`top_line`/`offset` (display col)/`tab`/`cursor_*`/`linums`/`syntax`/`viewmode` + `contentCursor`/`bodyLine`/`bodyLineCount`/`lineAttrRow` + `paint.paintBody` → `render.lgenLine`/`lgenPoint` + per-byte `attr_buf` (`render/attr.zig` merge/fromHybridRow) + optional JSF `syntax` fill + markdown `viewmode` (`render/view.zig` `analyzeLine`/`bindScratch` → `Options.view`) + `paintLinum` (tabs/UTF-8/controls/wide clip; unit-tested; table padded box-drawing via `render/table.zig`; no mark/OSC 8 yet).
...
- 🚧 `src/render/{root,lgen,gap,attr,syntax,view,table}.zig` — `lgen*` + `GapBuffer`/`Point` + per-byte `attr_buf` apply + Zig-native JSF subset DFA (`load`/`loadWithLibrary`/`parseLine`/`stateAfterLines`; `conf.jsf` + keywords/`buffer`/`strings`/`"&"` + local/external `call=`/`return` + `.ifdef` params + `reset` + mark/recolormark + `\i`/`\c` + delimiter `save_*`/`push_*`/`pop_*`/`%`/`&`) + Zig-native `lgen_view` Feature 1.x (`ViewTables` hide/substitute/link_url/col_map; headings/fences/HR/blockquote/task/emphasis/links; `Options.view` + `paintBody`/`TextWindow.viewmode` via stack `bindScratch`) + Feature 2.1/2.2 table box-drawing (`table.zig` region detect, col widths/align, padded `│`/`├─┼─┤` paint; `paintBody` prefers table rows); `TextWindow.buffer`/`syntax`/`line_attrs`/`viewmode` + `paintBody` auto-fill; `zig build render-test` **58/58**; `window-test` **90/90**; still no OSC 8/`hold`/lattr cache/hybrid-B/live joe wiring
...
- ⬜ JSF deepen next: lattr-style state cache, `hold`
...
- ⬜ `lgen_view` deepen: OSC 8 hyperlink emit, `hold`, lattr-style state cache
