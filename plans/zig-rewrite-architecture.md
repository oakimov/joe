...
- ✅ Path A thin `bwgen` entry: always-on `zig_bw_bwgen_entry` owns lattr/viewmode/mark setup + paint loops + Feature 1.10 cursor; mark setup Zig-owned (thin C field accessors); C `bwgen` aborts on `-1`
- ✅ Path A thin `bwgenh`: gated `zig_bw_bwgenh` owns hex dump rows; C keeps mark setup; loop calls C `genfield` + `zig_c_bw_pbyte`/`pgetb`
- ✅ Path A thin `bwgenh` entry: always-on `zig_bw_bwgenh_entry` owns mark setup + hex paint; mark setup Zig-owned (thin C field accessors); C `bwgenh` aborts on `-1`
...
- ✅ Path A `bwgenh` entry: always-on `zig_bw_bwgenh_entry` (Zig-owned mark setup + paint; abort on `-1`)
...
- ✅ Path A lifecycle: always-on `zig_bw_bwmove` / `zig_bw_bwresz` / `zig_bw_bwmk` / `zig_bw_bwrm` / `zig_bw_orphit` / `zig_bw_calclincols` (Zig-owned `bwMkInit`/`bwOrphit`/`bwrm`; remaining C helpers for field/VT resize; C fallback bodies deleted — abort on `-1`)
- ✅ Path A non-paint helpers: always-on `zig_bw_get_file_pos` / `zig_bw_set_file_pos` / `zig_bw_save_file_pos` / `zig_bw_load_file_pos` / `zig_bw_set_file_pos_all` / `zig_bw_vtmaster` / `zig_bw_ustat` / `zig_bw_ucrawlr` / `zig_bw_ucrawll` / `zig_bw_init_visiblews` (Zig-owned file_pos LRU + TW window walk + `vtmaster`/`ustat`/`windBw`; C public fallback bodies deleted — abort on `-1`)
...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + `bwgen`/`bwgenh` entry + table detect + follow + post-edit scroll + full `lgen_view` Feature 1.x + thin `lgen_view` dispatcher/entry + Zig-owned viewmode statics (`zig_bw_vm_*`) + C Feature chrome + follow/scroll/lifecycle/helper + `lgen_core`/`gennum` safety nets retired (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.3–1.10 / `bwmk`/`bwmove`/`bwresz`/`bwrm`/`orphit`/`calclincols` / `file_pos*` / `ustat` / `ucrawl*` / `vtmaster` / `init_visiblews` / Zig-owned `bwgen`/`bwgenh` mark setup / Zig-owned file_pos LRU); live `bw` is zero-C: `joe/bw.c` removed from the link; Zig `bw_lgen.zig` exports the JOE `bw.h` ABI + Path A paint/lifecycle/helpers (thin `lgen`/`lgen_core`/`gennum`/`bwgen*` wrappers abort on Zig `-1`; `defatr` in Zig)
...
- ✅ Path A `bwmk` init port: Zig `bwMkInit` owns BW alloc/init (status geometry, orphan restore, main-window kbd); deleted `zig_c_bw_alloc` / `zig_c_bw_mk_init`; soak **196/196**; `joe/bw.c` ~863 lines
- ✅ Path A file_pos window-walk port: Zig `zig_bw_set_file_pos_all` owns TW walk + `set_file_pos_orphaned`; deleted `zig_c_bw_file_pos_all`; soak **196/196**; `joe/bw.c` ~847 lines
- ✅ Path A `vtmaster` port: Zig `zig_bw_vtmaster` owns TW walk + VT cursor-byte match (typed `VtRec`); deleted `zig_c_bw_vtmaster_impl`; soak **196/196**; `joe/bw.c` ~834 lines
- ✅ Path A `orphit`/`bwrm` port: Zig `bwOrphit`/`bwRmSavePos`/`bwRmRelease` own orphan + destroy; deleted `zig_c_bw_orphit_impl`/`is_sole_errbuf`/`rm_*`; soak **196/196**; `joe/bw.c` ~807 lines
- ✅ Path A `ustat` port: Zig `bwUstat`/`windBw` own status-line command + TW/PW window cast (typed `Watom`); deleted `zig_c_bw_ustat_impl`/`zig_c_bw_wind_bw`; soak **196/196**; `joe/bw.c` ~779 lines
- ✅ Path A typed accessor + resize port: Zig owns BW/W/P field getters/setters + `bwmove`/`bwresz` helpers (`set_pos`/`set_size`/`dirty_grown_rows`/`resz_vt_if_master`); deleted matching `zig_c_bw_*` bridges; soak **196/196**; `joe/bw.c` ~606 lines
- ✅ Path A remaining helper-bridge port: Zig owns P-nav/`pcol`, SCRN field accessors + `nscrl*`/`msetI`/`updall`, lattr/viewmode/`get_highlight_state`, locale/`from_uni`; C keeps only `zig_c_bw_lgen`/`zig_c_bw_gennum` viewmode dispatch; soak **196/196**; `joe/bw.c` ~501 lines
...
### Phase 7: File I/O & Commands (3-4 weeks) — IN PROGRESS (Path A uedit + ublock + uformat + ushell + utag + umath + uisrch + usearch + uerror + ufile + tab + path done)
...
- ✅ Path A `ublock` rect/block-ops slice: Zig owns `pextrect`/`pdelrect`/`pclrrect`/`ptabrect`/`pinsrect` + `ublkdel`/`upicokill`/`ublkmove`/`ublkcpy`; soak **196/196**
...
- ✅ Path A `uerror` zero-C: Zig `src/uerror.zig` owns error-list parse/nav (`unxterr`/`uprverr`/`uparserr`/`ugparse`/`urelease`/`ujump`/`ucurrent_msg`/`parserrb` + `inserr`/`delerr`/`abrerr`/`saverr`/`beafter`/`parseone_grep`/`kill_ansi` + `errbuf`/`parserr_homeonly`); `joe/uerror.c` removed from the link (tombstone); soak **196/196**
- ✅ Path A `ufile` zero-C: Zig `src/ufile.zig` owns load/save/exit cmds (`usave`/`usavenow`/`ublksave`/`uexsve`/`uedit`/`uswitch`/`ulose`/`ubufed`/`uquerysave`/`ukilljoe`/`ureload`/`ureload_all` + `dosave`/`backup`/`yncheck`/`genexmsg` + file globals); `joe/ufile.c` removed from the link (tombstone); soak **196/196**
...
- ✅ Path A `path` zero-C: Zig `src/path.zig` owns directory/path helpers (`joesep`/`namprt`/`dirprt`/`mkpath`/`mktmp`/`rmatch`/`rexpnd*`/`chpwd`/`pwd`/`xdg_*`/`open_*_file`); `joe/path.c` removed from the link (tombstone); soak **196/196**; next Phase 7: remaining C (`help`/`poshist`/`mouse`/`gettext`/`builtins`/`vt`/`state`/`cclass`/`selinux`/`unicat`/`main`) or gapbuffer `fileio` hardening
