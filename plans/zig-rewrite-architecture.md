...
- ✅ Path A cursor follow: gated `zig_bw_bwfllwt` / `zig_bw_bwfllwh` (vertical recenter/`opt_mid`, horizontal `opt_left`/`opt_right`, hiline dirty; C scroll helpers + fallback retained)
- ✅ Path A post-edit scroll: gated `zig_bw_bwins` / `zig_bw_bwdel` (highlight dirty + `nscrldn`/`nscrlup`/`sary`; C fallback retained)
...
- ✅ Path A `gennum`: gated `zig_bw_gennum` mirrors C `" %21lld "` trailing `lincols` via hybrid `outatr` (same `JOE_ZIG_BW_LGEN` gate; C fallback retained)
...
- ✅ Path A thin `lgen_view` entry: gated `zig_bw_lgen_view_entry` owns prelude (len-cap/`parse`/line buffer) + dispatcher + paint cleanup; under the gate paints via Zig-owned `zig_bw_vm_*` tables (`paintViewBodyWithVm` → `zig_bw_lgen`); C keeps Feature fallback body + `zig_c_bw_view_{defatr,col_map*}` when gate off / Path A returns `-1`
...
- ✅ Path A dead `zig_c_bw_view_*` wrapper shrink: removed unused `paint_body`/`prepare`/`hide`/`subst`/`urls`/table-ptr/`after` bridges from `joe/bw.c` + matching Zig externs; kept `zig_c_bw_view_defatr` + `zig_c_bw_view_col_map{,_size,_line_ptr}` for Feature 1.10 cursor fallback
- ✅ Path A lifecycle: gated `zig_bw_bwmove` / `zig_bw_bwresz` / `zig_bw_bwmk` / `zig_bw_bwrm` / `zig_bw_orphit` / `zig_bw_calclincols` (C helpers for field/VT/kbd/file-pos; C fallback retained)
- ✅ Path A non-paint helpers: gated `zig_bw_get_file_pos` / `zig_bw_set_file_pos` / `zig_bw_save_file_pos` / `zig_bw_load_file_pos` / `zig_bw_set_file_pos_all` / `zig_bw_vtmaster` / `zig_bw_ustat` / `zig_bw_ucrawlr` / `zig_bw_ucrawll` / `zig_bw_init_visiblews` (C keeps file_pos list + window-walk/VT/status helpers; C fallback retained)
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + `bwgen` entry + table detect + follow + post-edit scroll + full `lgen_view` Feature 1.x + thin `lgen_view` dispatcher/entry + Zig-owned viewmode statics (`zig_bw_vm_*`) + C Feature chrome shrink (entry-only under gate) + dead-wrapper shrink + lifecycle + non-paint helpers (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.3–1.10 / `bwmk`/`bwmove`/`bwresz`/`bwrm`/`orphit`/`calclincols` / `file_pos*` / `ustat` / `ucrawl*` / `vtmaster` / `init_visiblews` / `bwgen` setup entry); still C: pure Feature fallback body + parallel C viewmode statics in `joe/bw.c` (live Zig bridges only `zig_c_bw_view_{defatr,col_map*}`)
