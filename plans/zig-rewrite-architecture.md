...
- ✅ Native edupd-shaped pass: `paint.paintAll` / `paintWindow` / `paintText` + `Screen.update` walk on-screen order → cells + msgs (unit-tested; text body still Phase 6/`render`).
