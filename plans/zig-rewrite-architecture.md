...
- 🚧 Phase 3 redesign started: Zig-native `src/terminal/{terminfo,tty,pty,screen}.zig` scaffolded + unit-tested (`zig build terminal-test` **14/14**); UTF-8 `writeText`, `Key`/`KeyParser`/`readKey` CSI+SS3 parsing, terminfo rare-cap cache; not yet replacing hybrid tty/termcap/scrn in the live binary
...
- ✅ `terminal/terminfo.zig` — Zig-native terminfo binding (`setupterm`/`tiget*` + cached rare caps `cup`/`clear`/`smcup`/…; unit-tested; not wired into live joe yet)
- ✅ `terminal/tty.zig` — Zig-native raw mode + size + SIGWINCH helper + `Key`/`KeyParser`/`readKey` (CSI/SS3/UTF-8; unit-tested; parallel to hybrid `src/tty.zig`)
- ✅ `terminal/pty.zig` — Zig-native `openpty`/`forkpty`/`login_tty` wrappers (unit-tested)
- ✅ `terminal/screen.zig` — Zig-native cell grid + dirty-row flush via ANSI + UTF-8 `writeText` (obuf-style out buffer; cell-diff deferred to Phase 6; unit-tested)
