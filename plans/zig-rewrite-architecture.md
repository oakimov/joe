...
- ✅ Path A dead `zig_c_bw_view_*` wrapper shrink: removed unused `paint_body`/`prepare`/`hide`/`subst`/`urls`/table-ptr/`after` bridges from `joe/bw.c` + matching Zig externs; kept `zig_c_bw_view_col_map{,_size,_line_ptr}` for Feature 1.10 cursor fallback
