...
- ✅ Path A Feature 2.2 padded table rows: gated `zig_bw_table_row` with C widths/aligns + `viewmode_col_map` fill; C fallback retained
...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex); still C: rest of `lgen_view` region detect + scrolling / window chrome helpers in `joe/bw.c`
