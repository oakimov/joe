...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + `bwgen` entry + table detect + follow + post-edit scroll + full `lgen_view` Feature 1.x + thin `lgen_view` dispatcher/entry + Zig-owned viewmode statics (`zig_bw_vm_*`) + C Feature chrome + follow/scroll/lifecycle/helper safety nets retired (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.3–1.10 / `bwmk`/`bwmove`/`bwresz`/`bwrm`/`orphit`/`calclincols` / `file_pos*` / `ustat` / `ucrawl*` / `vtmaster` / `init_visiblews` / `bwgen` setup entry); still C: `lgen_core`/`gennum`/mark-setup/`file_pos` list helpers + Zig bridge helpers in `joe/bw.c` (thin public wrappers abort on Zig `-1`; `defatr` in Zig)
