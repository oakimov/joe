...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect); still C: remaining `lgen_view` chrome + scrolling / window helpers in `joe/bw.c`
