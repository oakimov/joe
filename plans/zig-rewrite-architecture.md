...
- 🚧 Phase 5 redesign start: Zig-native `src/window/{root,screen,tw,pw,qw,menu}.zig` scaffold + layout (`create`/`layout`/`next`/`prev`/`close`/`resize`/`windowAt`); `zig build window-test` **9/9**; not wired into live joe
...
- 🚧 redesign `src/window/{root,screen,tw,pw,qw,menu}.zig` — Zig-native window types started (Screen+layout+kind stubs; `zig build window-test` **9/9**; hybrid ports remain live)
