...
- ✅ Path A thin `lgen_view` entry: gated `zig_bw_lgen_view_entry` owns prelude (len-cap/`parse`/line buffer) + dispatcher + paint cleanup; C keeps viewmode static storage helpers (`zig_c_bw_view_*`) + Feature fallback body + lifecycle helpers
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect + follow + post-edit scroll + full `lgen_view` Feature 1.x + thin `lgen_view` dispatcher/entry (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.3–1.10); still C: viewmode static storage + Feature fallback chrome + lifecycle helpers in `joe/bw.c`
