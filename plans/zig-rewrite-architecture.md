...
- ✅ Path A lifecycle: always-on `zig_bw_bwmove` / `zig_bw_bwresz` / `zig_bw_bwmk` / `zig_bw_bwrm` / `zig_bw_orphit` / `zig_bw_calclincols` (Zig-owned `bwMkInit` alloc/init; remaining C helpers for field/VT/`orphit`/`bwrm`; C fallback bodies deleted — abort on `-1`)
