...
- ✅ Native text-body via Phase 6: `TextWindow.body_lines` or live `buffer: *GapBuffer`/`top_line`/`offset` (display col)/`tab`/`cursor_*`/`linums` + `contentCursor`/`bodyLine`/`bodyLineCount` + `paint.paintBody` → `render.lgenLine`/`lgenPoint` + `paintLinum` (tabs/UTF-8/controls/wide clip; unit-tested; no syntax/viewmode/mark yet).
...
- 🚧 `src/render/{root,lgen,gap}.zig` — `lgenLine` over UTF-8 slices + Zig-native `GapBuffer`/`Point` + `lgenPoint`/`lgenBuffer` (tab expand, display-column `offset`, C0/DEL caret glyphs, wide-char `>` clip; live gap walk); `TextWindow.buffer` + `paintBody` prefer `lgenPoint`; `zig build render-test` **14/14**; `window-test` **86/86**; no syntax/`attr_buf`/`lgen_view`/hybrid-B yet
- ⬜ `lgen_core` syntax `attr_buf` (+ optional hybrid `B`/`P` bridge)
