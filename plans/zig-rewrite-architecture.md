...
- ✅ Path A body + gutter + thin `bwgen`/`bwgenh` + table detect + follow + post-edit scroll + `lgen_view` Feature 1.x (UTF-8 + byte charmaps / marks / viewmode tables / visiblews / square / ansi / Feature 2.1 residual / Feature 2.2 / `gennum` / paint loops / hex / region detect / `bwfllw*` / `bwins`/`bwdel` / Feature 1.3–1.8); still C: Feature 1.9 fallback highlight + `lgen_view` orchestration/glue + lifecycle helpers in `joe/bw.c`
