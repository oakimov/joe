...
- 🚧 `src/render/{root,lgen,gap,attr}.zig` — `lgenLine`/`lgenPoint`/`lgenBuffer` + Zig-native `GapBuffer`/`Point` + per-byte `attr_buf` apply (`Options.attrs`, lead-byte UTF-8, `mergeHighlight`/`fromHybridRow`); `TextWindow.buffer`/`line_attrs` + `paintBody` wired; `zig build render-test` **20/20**; `window-test` **87/87**; no live JSF DFA parse/`lgen_view`/hybrid-B yet
- ✅ `lgen` syntax `attr_buf` consumption (native Attribute rows; hybrid packed-int conversion via `fromHybridRow`)
- ⬜ Live JSF DFA → `attr_buf` fill (bridge hybrid `syntax.parse` or native port) + optional hybrid `B`/`P` bridge
